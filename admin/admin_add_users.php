      ...
      <tr ng-repeat="user in allUsers | filter:userSearch">
        ...
        <td>
          ...
          <span ng-if="user.status != 0">
            ...
            <a href="admin_add_users/users/user_delete.php?id={{user.id}}"
      ...
      <table class="user-table">
        ...
        <tbody>
          <tr ng-repeat="login in alllogin | filter:userSearch">
            ...
            <td>{{login.U_id}}</td>
            <td ng-style="{
                'color': login.actions === 'LOGOUT' ? 'red' :
                        login.actions === 'DISABLED' ? 'blue' :
                        login.actions === 'LOGIN' ? 'green' :
                        'black',
                'font-weight': login.actions === 'DISABLED' ? 'bold' : 'normal'
              }">
              {{
              login.actions === 'LOGOUT' ? 'Inactive' :
              login.actions === 'DISABLED' ? 'Disabled' :
